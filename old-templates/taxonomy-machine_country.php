...
<body <?php body_class( $class = 'pageSearch' ); ?>>
	...
	<div class="fullWidth_wrapper searched_page_wrapper">
			...
			<div class="site_container">
				<ul>
					...
					<?php
						...
						<?php } else {?>
							<li id="currentCountry"><a href="<?php echo get_category_link($category->term_id); ?>"><?php echo $category->name; ?></a></li>
						<?php }
					?>
			...
			<div class="twoCatsFilters">
				<div class="twoCatsFiltersInner">
					<div class="sByCats">
						...
						<ul class="progress_category" style="display: none;">
							<?php
		...
		<!-- <button id="loadMore" data-currentpage="1" data-currentpageurl="<?php echo admin_url( 'admin-ajax.php' ); ?>">Load More</button> -->
	</div>
	...
	<!-- <script>
		jQuery(document).ready(function($) {
			$(document).on('click', '#loadMore', function(event) {
				...
				var currentpageurl = $(this).data('currentpageurl');
				var grabCategory = $("#currentCategory").text();
				...
				.done(function(data) {
					console.log(data);
					$(".allSearchedProducts").append(data);
					$("#loadMore").data('currentpage', currentpage+1);
					// Main Product Slider
					jquery('.pThumbSlider').slick({
						dots: true,
						arrows: true,
						prevArrow: '<span class="slick_prev"><i class="custom_left_arrow"></i></span>',
						nextArrow: '<span class="slick_next"><i class="custom_right_arrow"></i></span>'
					});
					jquery(".pThumbSlider .slick-dots li").off('click');
				})
				.fail(function(data) {
					console.log(data);
				});
