...
<body <?php body_class( $class = 'pageSearch' ); ?>>
			...
			<div class="site_container">
				<ul>
					...
					<?php
						...
						<?php } else {?>
							<li id="currentModel"><a href="<?php echo get_category_link($category->term_id); ?>"><?php echo $category->name; ?></a></li>
						<?php }
					?>
	...
	<script>
		...
		$(document).ready(function() {
			...
			// ======== Auto cats select =======
			var grabCatName = $(".searchBreadCrumbs ul li#currentModel a").text();
			// console.log(grabCatName);
			$("form.rawFilter .rawFModel").find("input[value="+grabCatName+"]").click();
			$("form.rawFilter .rawFModel").find("input[value="+grabCatName+"]").next().click();
		});
