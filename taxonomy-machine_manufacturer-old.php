...
<?php
	...
	if (isset($_GET['rawFState']) && !empty($_GET['rawFState'])) {
		$queryState = true;
...
<body <?php body_class( $class = 'pageSearch' ); ?>>
		...
		<div class="fullWidth_wrapper related_products_heading searchBreadCrumbs">
			<div class="site_container">
				<ul>
					...
					<li><a href="<?php $query -> the_post(); the_permalink(); ?>"><?php $currentIndustry = get_the_title(); the_title(); ?></a></li>

					<?php
						if ($queryMan == true) {?>
							<li id="currentManufacturer"><a href=""><?php echo $_GET['rawFMan']; ?></a></li>
						<?php } else {?>
					...
					<?php
						if ($queryCat == true) {?>
							<li id="currentCategory"><a href=""><?php echo $_GET['rawFCat']; ?></a></li>
						<?php }
					?>

					<?php
						if ($queryModel == true) {?>
							<li id="currentModel"><a href=""><?php echo $_GET['rawFModel']; ?></a></li>
						<?php }
					?>

					<?php
						if ($queryCountry == true) {?>
							<li id="currentCountry"><a href=""><?php echo $_GET['rawFCountry']; ?></a></li>
						<?php }
					?>

					<?php
						if ($queryState == true) {?>
							<li id="currentState"><a href=""><?php echo $_GET['rawFState']; ?></a></li>
						<?php }
					?>
				</ul>
			</div>
				...
				<ul>
					...
					<form action="<?php echo get_category_link($category->term_id); ?>" class="machine_filter">
						...
						<div class="rawFilterValues" style="display: none;">
							...
							<!-- Manufacturer -->
							<input type="text" name="rawFModel" value="<?php echo $rawFModel; ?>">
							<!-- <input type="text" name="rawFModelUrl" value="<?php //echo $rawFModelUrl; ?>"> -->
	...
	<script>
		...
		$(document).ready(function() {
			...
			// ===== Breadcrumb Clicks ====
			$(".searchBreadCrumbs ul li#currentCategory a, .searchBreadCrumbs ul li#currentModel a, .searchBreadCrumbs ul li#currentCountry a, .searchBreadCrumbs ul li#currentState a").click(function(event) {
				event.preventDefault();
			...
			if (
				...
				$(".sBycountry ul.progress_state").length == 1
			) {
			...
			if (
				...
				$(".twoCatsFilters").hide();
				$(".searchBreadCrumbs").css("margin-bottom", "87px");
			}
			...
			// ======= Send Industry info ======
			$("li#currentManufacturer a").click(function(event) {
				event.preventDefault();
		...
		$(document).ready(function() {
			...
			if ($(window).width() > 1024) {
				if ( $('.sByCats ul:visible').hasClass('progress_category') ) {
					$('.sByCats ul:visible li:gt(11), .sBycountry ul:visible li:gt(14)').hide();
				} else {
	...
	<script>
		$(document).ready(function() {
			...
			$("button.collapse_catsSec").click(function(event) {
				...
				if ( $(window).width() >= 1024 ) {
					if ( getActiveUl == 'progress_category' ) {
						verticalAlign('progress_category', 2, 0);
					} else {
					...
					if ( $(this).parent().attr('class') == 'sByCats' ) {
						...
						// console.log("Parent is: " + $(this).parent().attr('class') + "ul is: " + getActiveUl );
					} else {
