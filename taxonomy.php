...
<?php
	...
	if (is_tax( 'machine_category')) {
		$currentTaxo = 'Category';
	} elseif (is_tax( 'machine_manufacturer')) {
		$currentTaxo = 'Manufacturer';
	}	
...
<body <?php body_class( $class = 'pageSearch' ); ?>>
	...
	<div class="fullWidth_wrapper searched_page_wrapper">
				...
				<ul>
					...
					<?php
						if ($currentTaxo == 'Category') {
							...
							?>
							<li id="currentManufacturer"><a href="<?php echo get_category_link($category->term_id) . '?industryName='.$getIndustryName.''; ?>"><?php echo $category->name; ?></a></li>
							<?php
						}
					?>

					<?php
						if ($queryMan || $queryCat) {
							if ($queryMan) {
								?>
							<li id="currentManufacturer"><a href="" title="Manufacturer"><?php echo $rawFMan; ?></a></li>
							<?php } else {
								?>
							<li id="currentCategory"><a href="" title="Category"><?php echo $rawFCat; ?></a></li>
						<?php } }
					?>
					...
					<div class="sByCats">
						<?php
							if ($currentTaxo == 'Category') {
								...
								?>
								<h2>Search by Category <span class="line"></span></h2>
								<?php
							}
							...
							if ($currentTaxo == 'Category') {
								...
								if (!isset($_GET['rawFCat']) OR empty($_GET['rawFCat'])) {
									?>
						...
						<?php
							// Global array variables for model, country and state
							if ($currentTaxo == 'Category') {
								$createCategoryArray = array(
									'taxonomy' => 'machine_category',
									'field'    => 'name',
					...
					<form action="<?php echo get_category_link($category->term_id); ?>" class="machine_filter">
						...
						<li class="fByYear">
							...
							<div class="cusFYear_field">
								...
								<div class="sField">
									<img class="checkedF" src="" alt="">
									<input type="radio" id="Yopt4" name="fYear" value="2010 - 2020">
									<label for="Yopt4">2010 - 2020</label>
								</div>
								<div class="sField">
									<img class="checkedF" src="" alt="">
									<input type="radio" id="Yopt5" name="fYear" value="2020 - 2025">
									<label for="Yopt5">2020 - 2025</label>
								</div>
							</div>
						</li>
						<li class="fByPrice">
		...
		<div class="allSearchedProducts">
			<?php
				...
				$phpVariable['currentTaxo'] = $currentTaxo;
	...
	<script>
		...
		$(document).ready(function() {
			...
			var currentTaxo = jsData.currentTaxo;
			if (currentTaxo == "Category") {
				var menuTargets = ".searchBreadCrumbs ul li#currentManufacturer a, .searchBreadCrumbs ul li#currentModel a, .searchBreadCrumbs ul li#currentCountry a, .searchBreadCrumbs ul li#currentState a";
			} else if (currentTaxo == "Manufacturer") {
				var menuTargets = ".searchBreadCrumbs ul li#currentCategory a, .searchBreadCrumbs ul li#currentModel a, .searchBreadCrumbs ul li#currentCountry a, .searchBreadCrumbs ul li#currentState a";
			}

			$(menuTargets).click(function(event) {
				event.preventDefault();
			...
			if (currentTaxo == "Category") {
				...
				if ($(".searchBreadCrumbs ul li").length == 3 && $(".searchBreadCrumbs ul li#currentManufacturer").length == 1 && $(".searchBreadCrumbs ul li#currentCategory").length == 1) {
					$(".twoCatsFiltersInner ul.progress_category, .twoCatsFiltersInner ul.progress_manufacturer").hide();
					$(".twoCatsFiltersInner .sByCats > h2").text('Search by Model');
					$(".twoCatsFiltersInner .sByCats > h2").append('<span class="line" style="width: 704px;">');
					$(".twoCatsFiltersInner ul.progress_model").show();
	...
	<script>
		$(document).ready(function() {
			...
			// ======== Vertical allignment  ========
			function verticalAlign(secName, column = 3, plus = 0.4) {
			...
			var jsData = <?php echo $jsonData; ?>;
			var currentTaxo = jsData.currentTaxo;
			// console.log(currentTaxo);
			
			if ( $(window).width() > 1024 ) {
				if (currentTaxo == 'Category') {
					verticalAlign('progress_manufacturer');
				} else if (currentTaxo == 'Manufacturer') {
					verticalAlign('progress_category', 2, 0);
					$(".twoCatsFilters .twoCatsFiltersInner ul.progress_category").css('grid-template-columns', '1fr 1fr');
				}
				...
				if (currentTaxo == 'Category') {
					...
					verticalAlign('progress_category', 3, 0);
				}
				...
				if (currentTaxo == 'Category') {
					...
					verticalAlign('progress_category', 1, 0);
				}
